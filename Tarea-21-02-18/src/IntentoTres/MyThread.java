...
public class MyThread extends Thread {
    ...
    public void run() {
    	int cont  = 1;
    	IntentoTres intTres = new IntentoTres();

        while (true) {
        	
        	System.out.println("Iteracion : " + cont +" id : " + this.getId() + " bandera : " + intTres.bandera );
            try {
            	intTres.requestCR(myId);
				this.CriticalRegion();
            	intTres.releaseCR(myId);
    ...
    public static void main(String[] args) throws Throwable {
        ...
        for (int i = 0; i < N; i++) {
        	lock[i] = new IntentoTres();//cualquier otro algoritmo
